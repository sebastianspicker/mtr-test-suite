...
Linux/macOS full run (may require `sudo` for raw socket access):
```bash
sudo ./mtr-test-suite.sh
```
...
Test types:
- `ICMP4`/`ICMP6` -- standard ping probes (default MTR mode)
- `UDP4`/`UDP6` -- UDP probes (detects UDP-specific filtering)
- `TCP4`/`TCP6` -- TCP SYN probes on port 443 (useful when ICMP is blocked)
- `MPLS4`/`MPLS6` -- MPLS label discovery (`-e`)
- `AS4`/`AS6` -- AS number lookup per hop (`-z`)

Rounds vary MTR parameters (packet size, TTL, TOS, timeouts). Use `--list-rounds` to see all.
